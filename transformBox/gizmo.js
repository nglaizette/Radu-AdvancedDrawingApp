class Gizmo {
	...
	addEventListeners(startPosition, handle, selectedShapes) {
		...
		let mouseDelta = null;
		let prevRatio = null;
		const prevSize = {width: this.box.width, heith: this.box.height };
		...
		const upCallback = (e) => {
			...
			selectedShapes.forEach((s) => s.setSize(s.size.width, s.size.heigh));
			viewport
				.getStageCanvas()
	...
	draw(ctx, hitRegion = false) {
		...
		if (!hitRegion) {
			...
			const centerLenght = 2 * Math.PI * centerRadius;
			const dashCount = 7;
			const dashLenght = 0.25 * centerLenght / dashCount;
			const spaceLenght = 0.75 * centerLenght / dashCount;
			ctx.save();
			ctx.beginPath();
			ctx.lineWidth = 3 / viewport.zoom;
			ctx.setLineDash([dashLenght, spaceLenght]);
